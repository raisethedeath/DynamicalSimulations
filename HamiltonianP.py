...
class Potential:
    ...
    def __init__(self, args, angleEnergyArray):
        ...
        if self.args.method == 'python':
            ...
            self.start = time.time()
            np.savetxt("tmphamil.txt", np.zeros((self.dim, self.dim)))
            HamiltonianF.hamiltonian(self.args.J, 
                                         self.dim, 
                                         np.reshape(self.angleEnergyArray, self.angleEnergyArray.shape, order='F'), 
                                         self.angleEnergyArray.shape[0]
                                         )
            self.Hamil = np.loadtxt("hamil.tmp").reshape(self.dim, self.dim)
            os.system("rm hamil.tmp")       
        self.FinishRun()
            ...
            for row in range(col, self.dim):
                ...
                if tmpHamil[col, row] != 0.:
                    ...
                    if time_check == False:
                        ...
                        self.PrintTime(val, "Estimated time for Populating Rotational Hamiltonian Matrix")
